...
| 25 | 8/31 | "use the language that we use... connections, speed, responsiveness. If you need context, refer to lane 2" | APW VOCABULARY LAW: reports use house terms — speed to lead, direct to booking, connected, triaged, responsive/unresponsive, hung up, no-show, reheat. Never invented phrases like "worked"/"untouched". |
| 26 | 8/31 | "I should be able to see every stage, from beginning to the end, on one page, right at the beginning" | LAYOUT LAW: every funnel/report artifact opens with the complete picture (all stages, all segments, all names) on the first screen; details expand below via links/anchors. |
| 27 | 9/2 | Alan: "why are they invisible? teramind should have everyone, i can see their screens" | NEVER infer absence of coverage/monitoring/data from a SUMMARY or digest artifact. Digest emails show top-N, not rosters — proof: the Teramind Activity block omits Alan's own monitored machine. Coverage claims require the source system (dashboard/API), not a notification. Absence in a summary = unknown, never "not covered". |
| 28 | 9/3 | Classification precedes precision (Teramind) | An activity/productivity metric must be CALIBRATED before its frequency is raised. Unclassified apps + short idle timeouts score a 2-hour sales call as idle. Never present or act on an activity % whose app classification is unverified, and never mark social/job-search sites unproductive at APW without the role exceptions — they are work tools here. |
